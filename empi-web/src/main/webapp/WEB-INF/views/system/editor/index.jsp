...
<ewcms:head title="在线编辑"/>
	...
	<div id="tb" style="padding:5px;height:auto;">
		...
		<div id="menu-addsub" style="width:80px;">
            <div id="menu-add-directory" data-options="iconCls:'icon-directory'">目录</div>
...
$(function(){
			...
			if (r){
	            $.post('${ctx}/system/editor/rename',{'path': row.attributes.path, 'newName':r},function(data){
		            if(data.success){
		            	...
		            	row.attributes.path = row.attributes.parentPath + '\\' + row.text;
						$('#tt').treegrid('update', row);
	...
	$('#menu-add-directory').bind('click', function(){
		//判断是否选择了节点
		var row = $('#tt').treegrid('getSelected');
		if(!row){
			$.messager.alert('提示', '请选择需要操作的节点', 'info');
		...
		$.messager.prompt('新建目录', '<strong>注意：</strong><span>支持如/a/b/c多级目录</span><br/><br/>请输入目录名称', function(r){
			if (r){
	            $.post('${ctx}/system/editor/create/directory',{'parentPath': row.attributes.path, 'name':r},function(data){
		            $.messager.alert('提示', data.message, 'info');
	    	    });						
			}
		});
	...
	$('#menu-add-file').bind('click', function(){
		//判断是否选择了节点
		var row = $('#tt').treegrid('getSelected');
		if(!row){
			$.messager.alert('提示', '请选择需要操作的节点', 'info');
		...
		$.messager.prompt('新建文件', '<strong>注意：</strong><span>支持如/a/b.js多级目录</span><br/><br/>请输入文件名称', function(r){
			if (r){
	            $.post('${ctx}/system/editor/create/file',{'parentPath': row.attributes.path, 'name':r},function(data){
		            $.messager.alert('提示', data.message, 'info');
	    	    });						
			}
		});
