...
@Table(name = "sec_user",
		uniqueConstraints = {
			@UniqueConstraint(name = "unique_sec_user_user_name", columnNames = "user_name"),
...
public class User extends BaseEntity<Long> implements LogicDeleteable {
	...
	public String getUsernameAndRealname(){
		return this.username + (EmptyUtil.isStringNotEmpty(this.getRealname()) ? "(" + this.getRealname() + ")" : ""); 
	}
}