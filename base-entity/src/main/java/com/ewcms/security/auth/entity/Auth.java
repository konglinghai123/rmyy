...
public class Auth extends BaseEntity<Long> {
	...
	@Formula(value = "(select o.name from sec_organization o where o.id = organization_id)")
	private String organizationName;
	@Column(name = "job_id")
	private Long jobId = 0L;
	@Formula(value = "(select j.name from sec_job j where j.id = job_id)")
	private String jobName;
	@Column(name = "user_id")
	private Long userId = 0L;
	@Formula(value = "(select u.user_name + case when u.real_name is not null then ('(' + u.real_name + ')') else '' end from sec_user u where u.id=user_id)")
	private String userName;
	@Column(name = "group_id")
	private Long groupId = 0L;
	@Formula(value = "(select g.name from sec_group g where g.id = group_id)")
