...
import com.ewcms.system.scheduling.generate.common.ValidationErrors;
...
//	    PATTERN_TIMESTAMP_FORMAT = Pattern.compile("(\\p{L}|\\p{N}|(\\_)|(\\.)|(\\-))+");
    ...
    protected void validateJobTrigger(ValidationErrorsable errors, JobInfo job){
...
//            throw new JSException("jsexception.job.unknown.trigger.type", new Object[] {quotedTriggerType});
            throw new SchedulingException("scheduling.job.unknown.trigger.type", new Object[] {trigger.getClass().getName()});
        }
    }

    protected void validateJobSimpleTrigger(ValidationErrorsable errors, JobSimpleTrigger trigger) {
    	if (trigger.getOccurrenceCount() == null){
    		throw new SchedulingException("scheduling.job.count", null);
    	}
    ...
    protected void validateJobCalendarTrigger(ValidationErrorsable errors, JobCalendarTrigger trigger) {
        ...
        if (trigger.getDaysType().intValue() == JobCalendarTrigger.DAYS_TYPE_ALL.intValue()) {
            ...
            throw new SchedulingException("scheduling.job.unknown.trigger.days", null);
        }
