	...
	$(function(){
		 $("#tb-import").bind('click', function(){
			 ...
			 if($('#isDisabledOriginalData').val()=='true'){
				$.messager.confirm('提示', '确定要作废以前数据再导入吗？ <br/><font color="red">该操作会先删除数据库院用目录所有数据再导入新数据，请慎重操作！！！</font>', function(r) {
					if (r) {
				...
				$.messager.confirm('提示', '确定要增量导入新数据吗？<br/><font color="red">按照提取通用名，给药途径，编号，药品类型，院用目录通用名，生产企业，剂型,规格，包装数量,省招标通用名查重查重，重复记录的不保存</font>', function(r) {
					if (r) {
						document.forms[0].submit();
						$.ewcms.addLoading();
					}
