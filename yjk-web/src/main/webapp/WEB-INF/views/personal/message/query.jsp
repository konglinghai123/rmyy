...
<ewcms:head title="${state.info}"/>
	...
	<div id="tb" style="padding:5px;height:auto;">
		<div class="toolbar" style="margin-bottom:2px">
			<a id="tb-message" class="easyui-linkbutton" data-options="iconCls:'icon-subscription-add',plain:true" onclick="$.ewcms.add({src:'${ctx}/personal/message/send',title:'发送消息',width:800,height:450});" href="javascript:void(0);">发送新信息</a>
			...
			<a id="tb-clear" class="easyui-linkbutton" data-options="iconCls:'icon-remove',plain:true" onclick="$.ewcms.remove({title:'清空${state.info}',src:'${ctx}/personal/message/clear/${state}'});" href="javascript:void(0);">清空${state.info}</a>
		</div>
	...
	function formatterTitle(val, row){
		<c:choose>
			<c:when test="${state eq 'draft_box'}">
				var url = "${ctx}/personal/message/draft/" + row.id + "/send";
				return '<a href="javascript:void(0);" onclick="$.ewcms.openWindow({src:\'' + url + '\',title:\'消息内容\',width:800,height:450});">' + val + '</a>';;
			</c:when>
			<c:otherwise>
				var url = "${ctx}/personal/message/" + row.id + "/content";
				var html = '<a href="javascript:void(0);" onclick="$.ewcms.openWindow({src:\'' + url + '\',title:\'消息内容\',width:800,height:450});">' + val + '</a>';
				if (!row.read){
					html += '<span>未读</span>';
				}
				return html;
