...
import com.ewcms.hzda.service.RenalTransplantService;
import com.ewcms.hzda.web.controller.util.HzdaUtil;
import com.ewcms.security.user.entity.User;
import com.ewcms.security.user.web.bind.annotation.CurrentUser;

@Controller
@RequestMapping(value = "/hzda/renaltransplant")
public class RenalTransplantController extends BaseCRUDController<RenalTransplant, Long> {

	private RenalTransplantService getRenalTransplantService() {
	...
	public String showSaveForm(@CurrentUser User user,
		...
		RenalTransplant renalTransplant = getRenalTransplantService().findByGeneralInformationId(generalInformation.getId());
		if (renalTransplant == null) {
			renalTransplant = new RenalTransplant();
		}
		model.addAttribute("generalInformationId", generalInformation.getId());
		model.addAttribute("m", renalTransplant);
	...
	public String save(Model model, RenalTransplant m, BindingResult result, List<Long> selections) {
		throw new RuntimeException("discarded method");
	}

	@RequestMapping(value = "save/{generalInformationId}", method = RequestMethod.POST)
	public String save(Model model, @Valid @ModelAttribute("m") RenalTransplant m, BindingResult result, @CurrentUser User user, @PathVariable(value = "generalInformationId") GeneralInformation generalInformation, RedirectAttributes redirectAttributes) {
