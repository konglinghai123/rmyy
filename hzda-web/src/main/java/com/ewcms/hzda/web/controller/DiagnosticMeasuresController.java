...
import com.ewcms.hzda.service.DiagnosticMeasuresService;
import com.ewcms.hzda.web.controller.util.HzdaUtil;
import com.ewcms.security.user.entity.User;
import com.ewcms.security.user.web.bind.annotation.CurrentUser;

@Controller
@RequestMapping(value = "/hzda/diagnosticmeasures")
public class DiagnosticMeasuresController extends BaseCRUDController<DiagnosticMeasures, Long> {
	private DiagnosticMeasuresService getDiagnosticMeasuresService() {
		return (DiagnosticMeasuresService) baseService;
	}
	...
	public String showSaveForm(@CurrentUser User user,@PathVariable(value = "generalInformationId") GeneralInformation generalInformation, Model model, RedirectAttributes redirectAttributes) {
		...
		DiagnosticMeasures diagnosticMeasures = getDiagnosticMeasuresService().findByGeneralInformationId(generalInformation.getId());
		if (diagnosticMeasures == null) {
			diagnosticMeasures = new DiagnosticMeasures();
		}
		model.addAttribute("generalInformationId", generalInformation.getId());
		model.addAttribute("m", diagnosticMeasures);
	...
	public String save(Model model, DiagnosticMeasures m, BindingResult result, List<Long> selections) {
		throw new RuntimeException("discarded method");
	}

	@RequestMapping(value = "save/{generalInformationId}", method = RequestMethod.POST)
	public String save(Model model, @Valid @ModelAttribute("m") DiagnosticMeasures m, BindingResult result, @CurrentUser User user, @PathVariable(value = "generalInformationId") GeneralInformation generalInformation, RedirectAttributes redirectAttributes) {
