...
import com.ewcms.hzda.service.FamilyService;
import com.ewcms.hzda.web.controller.util.HzdaUtil;
import com.ewcms.security.user.entity.User;
import com.ewcms.security.user.web.bind.annotation.CurrentUser;

@Controller
@RequestMapping(value = "/hzda/family")
public class FamilyController extends BaseCRUDController<Family, Long> {
	...
	public String showSaveForm(@CurrentUser User user,
		...
		Family family = getFamilyService().findByGeneralInformationId(generalInformation.getId());
		if (family == null) {
			family = new Family();
		}
		model.addAttribute("generalInformationId", generalInformation.getId());
		model.addAttribute("m", family);
	...
	public String save(Model model, Family m, BindingResult result, List<Long> selections) {
		throw new RuntimeException("discarded method");
	}

	@RequestMapping(value = "save/{generalInformationId}", method = RequestMethod.POST)
	public String save(Model model, @Valid @ModelAttribute("m") Family m, BindingResult result, @CurrentUser User user, @PathVariable(value = "generalInformationId") GeneralInformation generalInformation, RedirectAttributes redirectAttributes) {
