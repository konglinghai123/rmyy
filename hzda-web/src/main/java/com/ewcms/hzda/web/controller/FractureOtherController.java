...
import com.ewcms.hzda.service.FractureOtherService;
import com.ewcms.hzda.web.controller.util.HzdaUtil;
import com.ewcms.security.user.entity.User;
import com.ewcms.security.user.web.bind.annotation.CurrentUser;

@Controller
@RequestMapping(value = "/hzda/fractureother")
public class FractureOtherController extends BaseCRUDController<FractureOther, Long> {
	...
	public String showSaveForm(@CurrentUser User user,@PathVariable(value = "generalInformationId") GeneralInformation generalInformation, Model model, RedirectAttributes redirectAttributes) {
		...
		FractureOther fractureOther = getFractureOtherService().findByGeneralInformationId(generalInformation.getId());
		if (fractureOther == null) {
			fractureOther = new FractureOther();
		}
		model.addAttribute("generalInformationId", generalInformation.getId());
		model.addAttribute("m", fractureOther);
	...
	public String save(Model model, FractureOther m, BindingResult result, List<Long> selections) {
		throw new RuntimeException("discarded method");
	}

	@RequestMapping(value = "save/{generalInformationId}", method = RequestMethod.POST)
	public String save(Model model, @Valid @ModelAttribute("m") FractureOther m, BindingResult result, @CurrentUser User user, @PathVariable(value = "generalInformationId") GeneralInformation generalInformation, RedirectAttributes redirectAttributes) {
