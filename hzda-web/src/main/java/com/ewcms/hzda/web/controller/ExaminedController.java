...
import com.ewcms.hzda.service.ExaminedService;
import com.ewcms.hzda.web.controller.util.HzdaUtil;
import com.ewcms.security.user.entity.User;
import com.ewcms.security.user.web.bind.annotation.CurrentUser;

@Controller
@RequestMapping(value = "/hzda/examined")
public class ExaminedController extends BaseCRUDController<Examined, Long> {

	private ExaminedService getExaminedService() {
	...
	public String showSaveForm(@CurrentUser User user,
		...
		Examined examined = getExaminedService().findByGeneralInformationId(generalInformation.getId());
		if (examined == null) {
			examined = new Examined();
		}
		model.addAttribute("generalInformationId", generalInformation.getId());
		model.addAttribute("m",examined);
	...
	public String save(Model model, Examined m, BindingResult result, List<Long> selections) {
		throw new RuntimeException("discarded method");
	}

	@RequestMapping(value = "save/{generalInformationId}", method = RequestMethod.POST)
	public String save(Model model, @Valid @ModelAttribute("m") Examined m, BindingResult result, @CurrentUser User user, @PathVariable(value = "generalInformationId") GeneralInformation generalInformation, RedirectAttributes redirectAttributes) {
